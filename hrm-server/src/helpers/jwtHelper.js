    ...
    return new Promise((resolve, reject) => {
      JWT.verify(
        ...
        (err, payload) => {
          ...
          redisClient.GET(empId, (err, result) => {
            if (err) {
              return reject(
                createError.InternalServerError(
                  `Error while getting refreshToken against ${empId} from database`
                )
              );
            }
            if (refreshToken === result) {
              return resolve(empId);
            }
            return reject(
              createError.Unauthorized(
